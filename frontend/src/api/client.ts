import type { FilterParams, SearchResponse, Stats, SkillInfo, MiningResult, Transcript, TranscriptDetail, Project, Task, Source } from "./types";
...
export const api = {
  // Sources
  listSources: (params?: FilterParams): Promise<{ items: Source[]; total: number }> =>
    fetchJson(`${API_BASE}/sources?${buildQuery(params)}`),

  // Transcripts
  listTranscripts: (params?: FilterParams): Promise<{ items: Transcript[]; total: number }> =>
  ...
  // Projects
  listProjects: (params?: FilterParams): Promise<{ items: Project[]; total: number }> =>
    fetchJson(`${API_BASE}/projects?${buildQuery(params)}`),
  getProject: (id: string): Promise<Project & { tasks?: Task[]; transcripts?: Transcript[] }> =>
    fetchJson(`${API_BASE}/projects/${id}`),

  // Tasks
  listTasks: (params?: FilterParams): Promise<{ items: Task[]; total: number }> =>
  ...
  // Search
  search: (query: string, type = "hybrid", filters?: Record<string, unknown>): Promise<SearchResponse> =>
  ...
  // Todoist
  exportToTodoist: (projectId: string, todoistProjectId?: string): Promise<unknown> =>
